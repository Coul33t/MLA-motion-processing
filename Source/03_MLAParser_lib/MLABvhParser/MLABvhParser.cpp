...
/*
void BvhParser::parseHeader(const std::string& hierarchyFile, Motion* motion) {
	std::ifstream infile(hierarchyFile.c_str());
	...
	if (infile.is_open()) {
		...
		std::cout << "Failed to open " << hierarchyFile << std::endl;
	}
}

void BvhParser::parseInformations(const std::string& informationsFile, Motion* motion) {
	
	std::ifstream infile (informationsFile.c_str());
	...
	if (infile.is_open()) {
		...
		infile >> currentWord;
		int frameNumber = atoi(currentWord.c_str());
		...
		std::cout << "Failed to open " << informationsFile << std::endl;
	}
}

void BvhParser::parseFrames(const std::string& framesFile, Motion* motion) {
	std::ifstream infile(framesFile.c_str());
	...
	if(infile.is_open()) {
		...
		// (approximately 100 times faster), but why make a call to this 
		// function each time, rather than doing it only 1 time ?
		unsigned int jointNumber = motion->getFrame(0)->getJoints().size();
		unsigned int frameNumber = motion->getFrames().size();
		std::cout << "Copying frames ..." << std::endl;
		...
		for (unsigned int i = 0; i<frameNumber; i++) {
			...
			copiedFrame->setNames(motion->getFrame(0)->getNames());
			copiedFrame->setRoots(motion->getFrame(0)->getRoots());

			for (unsigned int i = 0; i<jointNumber; i++) {
				Joint* new_joint = new Joint(*motion->getFrame(0)->getJoints().at(i));

				new_joint->setParent(0);

				if (motion->getFrame(0)->getJoints().at(i)->getParent()) {
					std::string parentName = motion->getFrame(0)->getJoints().at(i)->getParent()->getJointName();
					new_joint->setParent(copiedFrame->getJoint(parentName));
				}
		...
		std::cout << "Failed to open " << framesFile << std::endl;
	}
}
*/
